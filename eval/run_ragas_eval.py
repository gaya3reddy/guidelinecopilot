...
----------------------
RAGAS evaluation harness for GuidelineCopilot.

Reads the golden dataset from eval/golden/who_hand_hygiene.json,
calls POST /ask for each question, collects answers + retrieved contexts,
then scores them with four RAGAS metrics:
...
Usage:
    ...
    EVAL_BASE_URL=http://localhost:8000 python -m eval.run_ragas_eval

Output:
    - Summary table printed to console
    - eval/reports/ragas_report_YYYYMMDD_HHMMSS.json  (full per-question scores)
    - eval/reports/ragas_baseline.json                (overwritten each run — used by CI)
"""
...
def run_eval() -> dict:
    ...
    timestamped_path = REPORTS_DIR / f"ragas_report_{timestamp}.json"
    timestamped_path.write_text(json.dumps(report, indent=2))
    print(f"\n  Full report saved → {timestamped_path}")

    # 11. Save/overwrite baseline (used by CI to check thresholds)
    baseline_path = PROJECT_ROOT / "eval" / "baseline" / "ragas_baseline.json"
